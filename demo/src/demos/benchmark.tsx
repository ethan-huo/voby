...
import { $, template, useSelector, For } from 'voby'
...
const Button = ({
	...
	text: FunctionMaybe<string>
	onClick: ObservableMaybe<(event: MouseEvent) => void>
}): JSX.Element => (
...
const Row = template(
	({
		...
		className: FunctionMaybe<Record<string, FunctionMaybe<boolean>>>
		onSelect: ObservableMaybe<(event: MouseEvent) => void>
		onRemove: ObservableMaybe<(event: MouseEvent) => void>
	}): JSX.Element => (
		<tr class={className}>
...
const Rows = ({
	...
	data: FunctionMaybe<IDatum[]>
	isSelected: (id: number) => FunctionMaybe<boolean>
}): JSX.Element => (
	<For values={data}>
		{(datum: IDatum) => {
