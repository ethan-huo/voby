...
const SOURCES = new WeakMap<object, Observable<any>>()
...
const hmr = <T extends Function>(
	...
	if (accept) {
		...
		if (isProvider) return component // Context providers are not hot-reloadable
		...
		const createHotComponentDeep = <U extends Function>(
			component: U,
			path: string[],
		): U => {
			const c = component as any
		...
		const onAccept = (module: Record<string, any>): void => {
			const hot =
				module[c[SYMBOL_AS]] || module[component.name] || module.default
		...
		;(cold as any)[SYMBOL_HOT_COMPONENT] = hot
		;(hot as any)[SYMBOL_COLD_COMPONENT] = cold
		;(hot as any)[SYMBOL_HOT_COMPONENT] = hot
		;(hot as any)[SYMBOL_HOT_ID] = id
