...
const hmrNamedExportAliasedRe =
	/^\s*([a-zA-Z$_][a-zA-Z0-9$_]*)\s+as\s+(_?[A-Z][a-zA-Z0-9$_-]*)\s*$/
const hmrNamedInlineExportRe =
	/^export\s+((?:function|const)\s+(_?[A-Z][a-zA-Z0-9$_-]*))/gm

export function voby(options: Options = {}): Plugin {
	...
	return {
		...
		transform: {
			...
			handler(code, id) {
				...
				code = code.replace(hmrDefaultExportRe, (_, $1: string, $2: string) => {
					return `export default $$hmr(import.meta.hot?.accept?.bind(import.meta.hot), ${$1})${$2}`
				})

				code = code.replace(
					hmrNamedInlineExportRe,
					(_, $1: string, $2: string) => {
						exports.push(
							`${$2}.__hmr_as__ = "${$2}";\n` +
				...
				code = code.replace(hmrNamedExportRe, (_, $1: string) => {
