...
type MaybePreloadableComponent = (() => Child) & {
	preload?: () => void
}

const [RouterContextProvider, useRouterContext] = createContext<RouterContext>(
	undefined,
	'Router',
)
const [RouteContextProvider, useRouteContext] = createContext<RouteContext>(
	undefined,
	'Route',
)
...
export const useRouter = () =>
	invariant(useRouterContext(), 'Make sure your app is wrapped in a <Router />')
...
export function createBranches(
	...
	return stack.length
		? branches
		: branches.sort((a: Branch, b: Branch) => b.score - a.score)
}
...
export function createLocation(
	...
	const url = useMemo<URL>(
		...
		{
			equals: (a: URL | undefined, b: URL | undefined) => a?.href === b?.href,
		},
	)
...
export function createRouterContext(
	...
	const parsePath = utils.parsePath || ((p: string) => p)
	const renderPath = utils.renderPath || ((p: string) => p)
