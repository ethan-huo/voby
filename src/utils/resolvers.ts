...
const resolveChild = <T>(
	...
	if (isFunction(value)) {
		...
		;(values as any)[SYMBOL_UNCACHED] = (value as any)[SYMBOL_UNCACHED] // Preserving this special symbol
...
const resolveStyle = (
	...
	if (isString(styles)) {
		...
		for (const key in styles) {
			...
			;(resolved as Record<string, unknown>)[key] = get(value)
		}
	}
