...
const setChildStatic = (
	...
	if (
		...
		(nextLength === 0 ||
			...
			(children as any)[SYMBOL_UNCACHED])
	) {
			...
			if (isStore(object)) {
				for (let i = 0, l = object.length; i < l; i++) {
					const fn = untrack(() =>
						isFunction(object[i])
							? object[i]
							: (object as any)[SYMBOL_STORE_OBSERVABLE](String(i)),
...
const setEventStatic = (() => {
	...
	const delegate = (event: string): void => {
		...
		document.addEventListener(event.slice(2), (event) => {
			...
			for (let i = 0, l = targets.length; i < l; i++) {
				...
				const handler = (target as any)[key]
	...
	return (
		...
		if (event.startsWith('onmiddleclick')) {
			...
			value =
				_value &&
				((event: Event) => (event as any)['button'] === 1 && _value(event))
		}

		const delegated = (delegatedEvents as any)[event]
...
const setPropertyStatic = (
	...
	if (key === 'value') {
		if (element.tagName === 'PROGRESS') {
			value ??= null
		} else if (element.tagName === 'SELECT' && !(element as any)['_$inited']) {
			;(element as any)['_$inited'] = true

			queueMicrotask(() => ((element as any)[key] = value))
		}
	}
